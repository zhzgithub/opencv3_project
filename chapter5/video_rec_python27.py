...
def read_images(path, sz=None):
    ...
    #每次遍历的对象都是返回的是一个三元组(root,dirs,files)	，dirs,files都是root路径下的目录和文件
    ...
    for dirname, dirnames, filenames in os.walk(path):  #filenames没用到
    ...
    while (True):
      ...
      for (x, y, w, h) in faces:
        ...
        try:
            ...
            print (roi.shape)
            params = model.predict(roi)			#[p_label, p_confidence] = model.predict(np.asarray(X[0]))
            print ("Label: %s, Confidence: %.2f" % (params[0], params[1]))
            cv2.putText(img, names[params[0]], (x, y - 20), cv2.FONT_HERSHEY_SIMPLEX, 1, 255, 2)
            if (params[0] == 0):	#如果标签为0，就写入一张图片
